...
class ExerciseAnalyzer:
    ...
    def __init__(
        ...
        if exercise_key not in EXERCISES:
            raise ValueError(
                f"Unknown exercise '{exercise_key}'. Options: {list(EXERCISES)}"
            )
    ...
    def process(self, frame_bgr) -> FrameResult:
        ...
        for angle_spec in self.spec.angles:
            ...
            if not visible_enough(a, b, c):
                feedback.append(
                    f"Can't see your {angle_spec.name.replace('_', ' ')} clearly - adjust the camera"
                )
                continue
        ...
        if self.spec.hold_target_angle is not None:
            ...
            if angle_value is not None and not (
                self.spec.hold_min_ok <= angle_value <= self.spec.hold_max_ok
            ):
                feedback.append(
                    "Straighten your body line - hips are sagging or piking up"
                )
    ...
    def _draw_hud(frame, angles, rep_count, hold_seconds, feedback) -> None:
        ...
        for msg in feedback[:2]:
            cv2.putText(
                frame, msg, (20, y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2
            )
            y += 25
